...
DiceOneLabel::DiceOneLabel(std::string text, std::string fontName, int fontSize, SDL_Color colour, glm::vec2 position, int fontStyle, bool isCentered) :
...
DiceOneLabel::~DiceOneLabel()
{

}

void DiceOneLabel::draw()
{
...
void DiceOneLabel::update()
{
}

void DiceOneLabel::clean()
{

}

void DiceOneLabel::setText(std::string newText)
{
...
void DiceOneLabel::setColour(SDL_Color newColour)
{
	FontManager::Instance()->load(m_fontPath, m_fontID, m_fontSize, m_fontStyle);
	FontManager::Instance()->textToTexture(m_text, m_fontID, m_fontID, newColour);
}

void DiceOneLabel::setSize(int newSize)
{
	m_fontSize = newSize;
...
void DiceOneLabel::m_buildFontID()
{
	m_fontID = m_fontName;
	m_fontID += "-";
